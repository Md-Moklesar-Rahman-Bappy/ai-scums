        ...
        <form method="POST" action="{{ route('teachers.store') }}">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input name="employee_id" class="form-control" value="{{ old('employee_id') }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Department</label>
                    <select name="department_id" class="form-select select2">
                        <option value="">Select</option>
                        @foreach(\App\Models\Department::all() as $d)<option value="{{ $d->id }}" @selected(old('department_id')==$d->id)>{{ $d->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input name="qualification" class="form-control" value="{{ old('qualification') }}">
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select">
                        @foreach(['active','inactive'] as $st)<option value="{{ $st }}" @selected(old('status','active')==$st)>{{ ucfirst($st) }}</option>@endforeach
                    </select>
                </div>
